...
class TaxHistoryCommand
{
    ...
    private function showList(int|string $chatId, int $offset): void
    {
        ...
        // Подтянем последние расчёты по всем компаниям пользователя, с жадной загрузкой company
        $q = TaxCalculation::query()
            ->with('company:id,name') // убедись, что в модели есть relation company()
            ->where('tg_user_id', $u->id)
            ->orderByDesc('created_at');

        $total = (clone $q)->count();

        $rows = $q->offset($offset)->limit(self::PAGE)->get();
        ...
        if ($offset + self::PAGE < $total) {
            $kb = [
                [
                    ['text' => 'Показать ещё', 'callback_data' => 'hist:more:' . ($offset + self::PAGE)],
                ],
            ];
        }
